...
// Multer config (Limit file size to 5MB to prevent crashes)
const upload = multer({ 
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // CRITICAL FIX: Ensure 'comparePassword' exists on User model
...
// POST Livestock (Admin) - with Validation
app.post('/api/admin/livestock', upload.single('image'), async (req, res) => {
    try {
        ...
        const newItem = new Livestock({
            name, type, breed, age, 
            price: parseFloat(price) || 0, // Prevent NaN crash
            tags: tagArray, 
            status, image
        });
...
app.put('/api/admin/livestock/:id', upload.single('image'), async (req, res) => {
    try {
        ...
        const { name, type, breed, age, price, tags, status } = req.body;
        const updates = { name, type, breed, age, price: parseFloat(price) || 0, status };
...
app.get('/api/livestock', async (req, res) => {
    try {
        const livestock = await Livestock.find({ status: 'Available' }, '-image'); // Exclude heavy image data for list
        res.json(livestock);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Global Error Handler (Catches Multer size errors, etc.)
